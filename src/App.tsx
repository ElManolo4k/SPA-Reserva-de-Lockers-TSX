...
// como el ID del locker, la fecha, la hora de inicio y fin, el usuario y el motivo.
  ...
  // updateLocker es una función que se encarga de actualizar los datos de un locker existente
  // permite cambiar el código, sede, edificio y piso de un locker sin cambiar su ID
  type UpdateLockerPayload = {
    id: string; // ID del locker a actualizar (no se puede cambiar)
    code: string;
    sede: SedeKey;
    edificio: string;
    piso: number;
  ...
  function updateLocker({ id, code, sede, edificio, piso }: UpdateLockerPayload) {
    const cleanCode = String(code ?? "").trim();
    const floor = Number(piso);

    if (!cleanCode || !sede || !edificio || !floor) {
      notify("error", "Completa todos los campos (código, sede, edificio, piso).");
      return;
    }
    ...
    if (!locker) {
      notify("error", "Locker no encontrado.");
      return;
    }

    setLockers((prev) =>
      prev.map((l) =>
        l.id === id ? { ...l, code: cleanCode, sede, edificio, piso: floor } : l
      )
    );
    notify("success", "Locker actualizado.");
  }

  // deleteLocker es una función que se encarga de eliminar un locker permanentemente
  // si el locker tiene reservas activas, se muestra un mensaje de error
  function deleteLocker(id: string) {
    ...
    const hasReservations = reservations.some((r) => r.lockerId === id && r.status === "CONFIRMADA");
    if (hasReservations) {
      notify("error", "No se puede eliminar un locker con reservas activas.");
      return;
    }

    if (!confirm(`¿Estás seguro de que quieres eliminar el locker ${locker.code} (${locker.id})?`)) return;

    setLockers((prev) => prev.filter((l) => l.id !== id));
    notify("success", "Locker eliminado.");
  }
  ...
  return (
    <div className="layout">
      ...
      <main className="main">
        ...
        {view === "lockers" && <LockersView lockers={lockers} onAdd={addLocker} onToggle={toggleLocker} onUpdate={updateLocker} onDelete={deleteLocker} />}
...
// con un formulario para crear nuevos lockers y botones para activar/desactivar, editar y eliminar cada locker.
function LockersView({
  ...
  onUpdate: (payload: { id: string; code: string; sede: SedeKey; edificio: string; piso: number }) => void;
  onDelete: (id: string) => void;
}) {
  // Estado para crear locker
  ...
  function submitCreate(e: React.FormEvent) {
    ...
    setId("");
    setCode("01");
    handleSedeChange("CAMPUS");
  }

  function submitEdit(e: React.FormEvent) {
    e.preventDefault();
    if (!editingId) return;
    onUpdate({ id: editingId, code: editCode, sede: editSede, edificio: editEdificio, piso: editPiso });
    cancelEdit();
  }
  ...
  return (
    <section className="card">
      ...
      <p className="muted">Crea, edita, activa/desactiva o elimina lockers. Asocia cada uno a sede, edificio y piso.</p>

      <hr className="hr" />

      <h3>➕ Crear nuevo locker</h3>
      <form className="row4" onSubmit={submitCreate}>
        <input value={id} onChange={(e) => setId(e.target.value)} placeholder="ID (ej: LK-SD-2-05)" required />
        <input value={code} onChange={(e) => setCode(e.target.value)} placeholder="Código visible (ej: 05)" required />
        ...
        <button type="submit" style={{ gridColumn: "auto" }}>Crear</button>
      </form>

      {editingId && (
        <>
          <hr className="hr" />
          <h3>✏️ Editar locker: <strong>{editingId}</strong></h3>
          <form className="row5" onSubmit={submitEdit}>
            <input value={editCode} onChange={(e) => setEditCode(e.target.value)} placeholder="Código (ej: 05)" required />

            <select value={editSede} onChange={(e) => handleEditSedeChange(e.target.value as SedeKey)}>
              {(Object.keys(SEDES) as SedeKey[]).map((k) => (
                <option key={k} value={k}>
                  {SEDES[k].label}
                </option>
            ...
            <select value={editEdificio} onChange={(e) => handleEditEdificioChange(e.target.value)}>
            ...
            <select value={editPiso} onChange={(e) => setEditPiso(Number(e.target.value))}>
            ...
            <div style={{ display: "flex", gap: "6px" }}>
              <button type="submit" style={{ flex: 1, padding: "8px 6px" }}>Guardar</button>
              <button type="button" onClick={cancelEdit} style={{ flex: 1, padding: "8px 6px", background: "#6b7280" }}>Cancelar</button>
            </div>
          </form>
        </>
      )}
        ...
        {lockers.length === 0 ? (
          <div className="trow">
            <div style={{ gridColumn: "1 / -1", textAlign: "center", color: "#999" }}>
              No hay lockers. Crea uno en el formulario de arriba.
            </div>
          </div>
        ) : (
          lockers.map((l) => (
            <div className="trow" key={l.id}>
              ...
              <div className="actions-cell">
                <button onClick={() => startEdit(l)} title="Editar">E</button>
                <button onClick={() => onToggle(l.id)} title={l.active ? "Desactivar" : "Activar"}>{l.active ? "D" : "A"}</button>
                <button className="danger" onClick={() => onDelete(l.id)} title="Eliminar">X</button>
              </div>
            </div>
          ))
        )}
